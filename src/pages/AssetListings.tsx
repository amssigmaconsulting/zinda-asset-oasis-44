      ...
      <CardContent className="p-6">
        ...
        <div className="flex gap-2">
          ...
          <Button asChild className="flex-1">
            <Link to={`/related-assets/${listing.id}`}>Similar Assets</Link>
          </Button>
        </div>
      </CardContent>
    </Card>
...
const AssetListings = () => {
  ...
  // Find the clicked asset to get its details
  const clickedAsset = allListings.find(listing => listing.id === parseInt(id || "0"));

  // Initialize filters based on the clicked asset
  useEffect(() => {
    if (clickedAsset && !searchParams.toString()) {
      setAssetType(clickedAsset.type);
      setLocation(clickedAsset.location);
      setCategory(clickedAsset.category);
    }
  }, [clickedAsset, searchParams]);
        ...
        <div className="container mx-auto px-4">
          <div className="mb-8">
            ...
            <h1 className="text-4xl font-bold bg-gradient-primary bg-clip-text text-transparent mb-2">
              {clickedAsset ? `${clickedAsset.type} Listings` : "Asset Listings"}
            </h1>
            <p className="text-muted-foreground text-lg">
              {clickedAsset 
                ? `Found ${filteredListings.length} ${clickedAsset.type.toLowerCase()} assets` 
                : `Found ${filteredListings.length} assets`}
            </p>
          </div>
